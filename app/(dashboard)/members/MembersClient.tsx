        ...
        {isFiltering && (
          <button
            ...
            className="absolute right-3.5 top-1/2 -translate-y-1/2 text-muted hover:text-text
              transition-colors duration-150 min-w-[24px] min-h-[24px] flex items-center justify-center"
          >
            ×
          </button>
        )}
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {filtered.map((member) => (
            <Link
              ...
              className={`card p-4 flex items-start gap-4 transition-shadow duration-100
                ${member.isFronting
                  ? 'border-front/40 shadow-front-glow hover:shadow-front-glow'
                  : 'hover:shadow-glow'
                }`}
            >
              <div
              ...
              <div className="flex-1 min-w-0">
                ...
                {member.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-2">
                    {member.tags.slice(0, 3).map((tag) => (
                      <span
                        key={tag}
                        className="text-xs bg-surface-alt text-muted px-2 py-0.5 rounded-full"
                      >
                        {tag}
                      </span>
                    ))}
                ...
                {member.isFronting && (
                  <div className="flex items-center gap-1.5 mt-2" aria-label="Currently fronting">
                    <span className="inline-flex h-2 w-2 rounded-full bg-front" />
                    <span className="text-xs font-semibold text-front tracking-wide">IN FRONT</span>
                  </div>
                )}
              </div>
            </Link>
