...
function sanitizePluralKitLinkMetadata(metadata, externalPluralKitId) {
  const source = isRecord(metadata) ? metadata : {};
  const safe = {};

  const shortId = cleanString(source.shortId);
  if (shortId) safe.shortId = shortId;

  const uuid = cleanString(source.uuid);
  if (uuid) safe.uuid = uuid;

  const displayName = cleanString(source.displayName);
  if (displayName) safe.displayName = displayName;

  const birthday = cleanString(source.birthday);
  if (birthday) safe.birthday = birthday;

  const normalizedExternalPluralKitId = cleanString(externalPluralKitId);
  if (normalizedExternalPluralKitId) {
    safe.externalPluralKitId = normalizedExternalPluralKitId;
  }

  return safe;
}

function sanitizeLinkMetadata(provider, metadata, externalPluralKitId) {
  if (provider === PROVIDERS.PLURALKIT) {
    return sanitizePluralKitLinkMetadata(metadata, externalPluralKitId);
  }
...
function buildLinkPayload(external) {
  return {
    ...
    metadata: JSON.stringify(sanitizeLinkMetadata(
      external.provider,
      external.metadata,
      external.externalPluralKitId,
    )),
  };
}
