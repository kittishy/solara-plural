...
function createPluralKitFrontSync(deps) {
  ...
  return async function syncFrontToPluralKit(systemId, memberIds, context = {}) {
    ...
    if (!response.ok) {
      ...
      // PluralKit returns HTTP 400 when the requested switch is already the
      // current switch. For Solara this is idempotent success, not a sync error.
      if (
        response.status === 400 &&
        normalizedMessage.includes('member list identical to current fronter list')
      ) {
        logger.info('[front-sync] pluralKit already matches local front', {
